...
# Flood Mapping Dashboard

Providing flood maps as web applications will make the information available with anyone who has internet access. The [Kansas Flood Mapping Dashboard](https://www.arcgis.com/apps/dashboards/dfa979db9f6f44fd8694bb1e9f53623a) (see below) is a web application that provides a simple interface to access the flood maps produced by the project.
...
## ArcGIS Dashboard Elements and Layout
...
* The [Kansas Flood Mapping Dashboard](https://www.arcgis.com/apps/dashboards/dfa979db9f6f44fd8694bb1e9f53623a) has an URL that's not easy to remember. Can we generate a more meaningful URL for the dashboard?

## ArcGIS Dashboard Issues
...
The issue is because <span style="color:red">**there is no way to control dashboard element's caching**.</span>. This is especially a problem for our application as it updates map services very often (for example, the stream flood map is updated every hour). Several approaches have been tried, but none of them seemed working.
...
* Publishing the gauges as a [Feature layer on the server](https://services.kars.geoplatform.ku.edu/arcgis/rest/services/fldpln_kansas/Gauges_Features/FeatureServer) (which allows user to edit features) seems not helpful though [this link](https://doc.arcgis.com/en/arcgis-online/manage-data/control-cache.htm) mentioned cache control for feature layer.
...
* Clear web browser caches and reload the app. Here it [how to clear caches in Chrome](https://support.google.com/accounts/answer/32050?hl=en&co=GENIE.Platform%3DDesktop). 
* See [this link](https://community.esri.com/t5/arcgis-web-appbuilder-questions/disable-client-caching-for-a-map-service-layer/td-p/179620) which asked for disable client caching for a map service layer, and <span style="color:red">**how to change some dashboard JavaScript code to handle the problem**</span>. 

## Streamlit Flood Mapping Dashboard
